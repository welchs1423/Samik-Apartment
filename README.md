...
│   └── AdminPage.vue           # 관리자 페이지 (로컬 개발 전용)
├── router/
│   └── index.js
├── App.vue
├── main.js
...
|---|---|---|
| `/` | 메인 히어로 | 전체 |
| `/menu` | 전체 카테고리 목록 + 이름 검색 | 전체 |
| `/menu/:id` | 카테고리별 아이템 (그리드 / 리스트 / 테이블 뷰 전환) | 전체 |
| `/admin` | 관리자 페이지 (비밀번호 보호) | **로컬 개발 전용** |
...
- 변경사항은 localStorage에 저장 (로컬 미리보기용)

## 콘텐츠 업데이트 워크플로우

```
1. npm run dev 실행
2. public/images/ 에 이미지 파일 추가
3. src/data/items/*.json 직접 수정 (또는 어드민에서 편집 후 JSON 수동 반영)
...
| 날짜 | 내용 |
|---|---|
| 2026-05-28 | 어드민 로컬 전용 전환, 이미지 git 직접 관리, 업로드 서버 제거 |
