...
- **Express** — 개발용 API 서버 (이미지 업로드 + JSON 파일 저장)
...
├── composables/
│   └── useMenuData.js          # 데이터 로드 + API 연동 reactive store
├── pages/
│   ├── MainPage.vue            # 메인 히어로
...
- 비밀번호 인증 (세션 유지)
- **카테고리** 추가 / 수정 / 삭제 + 커버 이미지 업로드
- **아이템** 추가 / 수정 / 삭제 — 이름, 설명, 재료, 태그, 이미지, 서브카테고리
...
Spirit 아이템은 최소한 `name`만 있으면 됩니다. 나머지 필드는 선택.

### items/{id}.json — 와인 (서브카테고리 그룹 구조)
...
|---|---|---|
| Vite | 5173 | 프론트엔드 HMR |
| Express (`server.js`) | 3001 | 이미지 업로드 + JSON 파일 저장 API |
